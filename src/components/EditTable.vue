...
const handleAppendColClick = ($index: number, ) => {
  // appendColumn($index, form);
    state.columns.splice($index + 1, 0, { prop: "", label: "" });
    ...
    <el-table border :data="state.tableData" style="width: 100%">
      ...
      >
        <template #header="scope">
          <el-input v-model="scope.column.label" style="margin-top: 1em;"></el-input>
          <el-button @click="handleAppendColClick(scope.$index)" link size="small" class="insert-column" type="primary">
            <AddColSvgIcon class="operate-btn" />插入列
          </el-button>
        </template>
      </el-table-column>
      <template #append>
